...
* You can [replace the boot logo](tools/bootLogoChanger.htm) with a custom one
* You can [replace the main menu music, or remove it entirely](sf2000/resources.md#sounds)
* You can [change the default button mappings](tools/buttonMappingChanger.htm) for each emulator (newer firmwares have this feature built-in, but the built-in implementation is buggy)
...
This was due to a bug in some earlier firmware versions; it was officially fixed in firmware 1.6, so if you're running a prior firmware version ([check here](tools/firmwareVersionChecker.htm)), you can [upgrade to a newer firmware version](sf2000/firmware.md#firmwarebios-bisrvasd).
